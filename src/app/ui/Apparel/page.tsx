...
import { Apparel_nav } from "@/app/constant"
...
export default async function Apparel () {
  const apparel_navigation = await prismaClient.product.findMany({
    where:{
       category:{
        slug:"apparel"
       }
  ...
  return(
    <div className="w-full mb-[40px]">
      ...
      <h1 className="text-[28px] font-[700] my-[15px] mx-[10px]">Category:Apparel</h1>
      <div className="flex grid grid-cols-1 lg:grid-cols-3 sm:grid-cols-2  ">
          {
            apparel_navigation.map((item,index) =>(
